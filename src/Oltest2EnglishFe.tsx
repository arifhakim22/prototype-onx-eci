...
function Oltest2EnglishFe() {
  ...
  const [pdfPaths, setPdfPaths] = useState<any | null>(null)
  ...
  const handleViewReport = (p: Participant) => {
    ...
    fetch(`/api2/get_reporting?id_user=${p.id_user}`)
      .then((res) => {
        if (!res.ok) throw new Error('API server returned error')
        return res.json()
      })
  ...
  const handleDownloadAll = () => {
    ...
    fetch(`/api2/update_download_status?peserta_id=${selectedUser.peserta_id}`, {
      method: 'POST',
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleStartBatch = async (batchNum: number) => {
    ...
    for (let i = 0; i < batchUsers.length; i++) {
      ...
      try {
        ...
        const response = await fetch(`/api2/get_reporting?id_user=${user.id_user}`);
        if (!response.ok) throw new Error(`API error for user ${user.id_user}`);

        // Step 2: Call API to update status in the database
        const updateRes = await fetch(`/api2/update_download_status?peserta_id=${user.peserta_id}`, {
          method: 'POST'
        });
      ...
      <aside className="w-[30%] min-w-[320px] max-w-[450px] border-r border-slate-200 flex flex-col h-full bg-slate-50/50 shrink-0">
        ...
        <div className="p-4 border-b border-slate-200 bg-white flex flex-col gap-3 shrink-0">
              ...
              {Array.from({ length: Math.ceil(participants.length / 100) }).map((_, i) => {
                ...
                return (
                  <button
                    ...
                    className={`cursor-pointer text-[11px] font-semibold py-1 px-1 rounded-md border text-center transition-all flex flex-col items-center justify-center gap-0.5 ${isProcessingBatch
                  ...
                  >
                    <span>#{i + 1}</span>
                    <span className={`text-[8px] uppercase tracking-wider font-mono font-bold ${activeBatch === i + 1 ? 'text-purple-200' : isDone ? 'text-emerald-600' : 'text-slate-400'
                      }`}>
                      {status}
                    </span>
                  </button>
            ...
            <div className="bg-purple-50 border border-purple-100 rounded-xl p-3 flex flex-col gap-1.5">
              <div className="flex justify-between items-center text-xs">
                <span className="font-semibold text-purple-800">
                  Processing Batch {batchProgress.batchNumber}...
                </span>
              ...
              <span className="text-[10px] font-mono text-slate-500 truncate leading-none mt-0.5">
                Processing: {batchProgress.currentUser}
              </span>
            </div>
          )}
          ...
          <div className="flex justify-between items-center text-[11px] text-slate-500">
            <span>Showing {filteredParticipants.length} user_real</span>
            {searchQuery && (
              <button
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2.5">
          {loading ? (
            ...
            <div className="text-center text-xs text-slate-400 py-12 italic">
              No participants match the query.
            </div>
          ) : (
      ...
      {!selectedUser ? (
        <main className="flex-1 h-full bg-slate-50/50 flex items-center justify-center p-8 text-center">
          <div className="max-w-sm flex flex-col items-center">
            ...
            <h3 className="text-sm font-bold text-slate-800 mb-1">No report selected</h3>
            <p className="text-xs text-slate-400">Select a participant on the left panel and click the "Report" button to load their report results.</p>
          </div>
        </main>
      ) : loadingReport ? (
        <main className="flex-1 h-full bg-slate-50/50 flex flex-col items-center justify-center p-8">
          ...
          <span className="text-xs text-slate-500 font-mono">Loading report for: {selectedUser.nama}...</span>
        </main>
      ) : (
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-8">
            {!reportingData || reportingData.length === 0 ? (
              <div className="bg-white rounded-2xl border border-slate-200/60 p-12 text-center shadow-sm">
                ...
                <p className="text-sm font-semibold text-slate-800">Empty Report</p>
                <p className="text-xs text-slate-400 mt-1">No personal_report data was found for this participant.</p>
              </div>
            ) : (
...
export default Oltest2EnglishFe;
