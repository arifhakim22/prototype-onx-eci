...
// Configure connection pool to local PostgreSQL database 'online_asset_english'
const pool = new Pool({
  host: 'localhost',
  port: 5432,
  user: 'meng',
  database: 'online_asset_english',
});
...
app.get('/api/get_list_peserta', async (_req, res) => {
  try {
    const result = await pool.query(`
      ...
      from m_paket_project_peserta_37 a 
...
async function generatePDFs(nama: string, reportItems: any[]): Promise<{ bahasaInggris: string | null }> {
  const dirPath = path.join(process.cwd(), 'public', 'pdf', 'test_bahasa_inggris', nama);
  if (!fs.existsSync(dirPath)) {
    fs.mkdirSync(dirPath, { recursive: true });
  }

  const pdfPaths = {
    bahasaInggris: `/pdf/test_bahasa_inggris/${encodeURIComponent(nama)}/Test_Bahasa_inggris_${encodeURIComponent(nama)}.pdf`
  };

  const filePaths = {
    bahasaInggris: path.join(dirPath, `Test_Bahasa_inggris_${nama}.pdf`)
  };

  // If PDF already exists, return paths immediately
  if (fs.existsSync(filePaths.bahasaInggris)) {
    return pdfPaths;
  }

  console.log(`[PDF Generator ENG] Generating PDF for user: ${nama}...`);
  ...
  try {
    ...
    for (const item of reportItems) {
      const isEnglishTest = item.namafile === 'Tes Bahasa Inggris' || item.namaFile === 'Tes Bahasa Inggris';

      if (!isEnglishTest) continue;

      const targetPath = filePaths.bahasaInggris;
...
app.post('/api/update_download_status', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      'UPDATE m_paket_project_peserta_37 SET status_download_pdf = 1 WHERE peserta_id = $1',
      [peserta_id]
    );
...
app.listen(PORT, () => {
  console.log(`[Backend Server ENG] Running on http://localhost:${PORT}`);
});
